...
import { EstadoRepositorio } from './components/EstadoRepositorio';
import { DismissButton } from './components/DismissButton';
import { login, dismissItem, getPR } from './db';
  ...
  return (
    <div>
      ...
      <div className="flex h-screen bg-gray-200">
        <div className={`fixed z-20 inset-0 bg-black opacity-50 transition-opacity lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`} onClick={() => setSidebar(false)}></div>
          ...
          <div className="flex items-center justify-center mt-8">
            <div className="flex items-center">
              <svg className="h-12 w-12" viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M364.61 390.213C304.625 450.196 207.37 450.196 147.386 390.213C117.394 360.22 102.398 320.911 102.398 281.6C102.398 242.291 117.394 202.981 147.386 172.989C147.386 230.4 153.6 281.6 230.4 307.2C230.4 256 256 102.4 294.4 76.7999C320 128 334.618 142.997 364.608 172.989C394.601 202.981 409.597 242.291 409.597 281.6C409.597 320.911 394.601 360.22 364.61 390.213Z" fill="#4C51BF" stroke="#4C51BF" stroke-width="2" strokeLinecap="round" strokeLinejoin="round" />
              ...
              <span className="text-white text-2xl mx-2 font-semibold">CCPR</span>
            </div>
          </div>

          <nav className="mt-10">
            <a className="flex items-center mt-4 py-2 px-6 border-l-4 bg-gray-600 bg-opacity-25 text-gray-100 border-gray-100" href="https://dashboard-tailwindcomponents.netlify.app/" target="_blank">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <header className="flex justify-between items-center py-4 px-6 bg-white border-b-4 border-indigo-600">
            <div className="flex items-center">
              <button className="text-gray-500 focus:outline-none lg:hidden" onClick={() => setSidebar(true)}>
              ...
              <div className="relative mx-4 lg:mx-0">
                ...
                <input value={search} onChange={(v) => setSearch(v.target.value)} className="form-input w-32 sm:w-64 rounded-md pl-10 pr-4 focus:border-indigo-600" type="text" placeholder="Search" />
              </div>
            </div>
            ...
            <div className="flex items-center">
              ...
              <div x-data="{ dropdownOpen: false }" className="relative">
                <button onClick={() => setDropdownOpen(!dropdownOpen)} className="relative z-10 block h-8 w-8 rounded-full overflow-hidden shadow focus:outline-none">
                              ...
                              <tr key={item.key}>
                                <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 font-medium">
                                  ...
                                  <div className="flex items-center">
                                    ...
                                    <div className="ml-4">
                                      <a href={'https://proyectos.andes.gob.ar/browse/' + item.key} target="_blank" className="text-indigo-600 hover:text-indigo-900">{item.key}</a>
                                      <div className="text-sm leading-5 text-gray-500">{item.user}</div>
                                    </div>
                                  </div>
                                ...
                                <td className="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                                  <DismissButton item={item} onPress={() => {
